...
void Capsule::drawDebug(Bengine::DebugRenderer& debugRenderer)
{
    ...
    glm::vec4 destRect(
        ...
        m_body->GetPosition().y - (m_dimensions.y - m_dimensions.x) / 2.0f,
    ...
    debugRenderer.drawCircle(
        glm::vec2(destRect.x + m_dimensions.x / 2.0f, destRect.y),
        color,
        m_dimensions.x / 2.0f
    );

    debugRenderer.drawCircle(
        glm::vec2(destRect.x + m_dimensions.x / 2.0f, destRect.y + destRect.w),
        color,
        m_dimensions.x / 2.0f
    );
}
